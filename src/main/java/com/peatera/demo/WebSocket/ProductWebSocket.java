...
@Component
@ServerEndpoint(value = "/productWebSocket/{dir}", configurator = MyEndpointConfigure.class)
public class ProductWebSocket {
     ...
     */
    @OnOpen
    public void onOpen(@PathParam("dir")String dir, Session session) {
        log.info("dir：" + dir);
        ...
        if(dir!=null) {
            ...
            totalPushMsgs.add(dir+"连接成功-"+"-当前在线人数为："+getOnlineCount());
 
 
            if(totalPushMsgs != null && !totalPushMsgs.isEmpty()) {
                //totalPushMsgs.forEach(e -> sendMessage(JSONObject.toJSONString(e)));
            }
        }
     ...
     */
    @OnMessage
    public void onMessage(String dir, Session session) {
    	
    	log.info("用户发送过来的dir为："+dir);
     ...
     * 群发自定义消息
     */
    public static void sendInfo(JSONObject message) throws IOException {
