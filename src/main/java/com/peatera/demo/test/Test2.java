    ...
    public String getDefaultIpAddress() { 
    	 ...
    	 try {
			...
			// 没有出现异常而正常当取到的IP时，如果取到的不是网卡循回地址时就返回  
			 ...
			 // hostname not in DNS or /etc/host
		}
    	  Sigar sigar = new Sigar();  
    	  try {
 ...
 // c)取到当前机器的MAC地址  
    public String getMAC() {  
         ...
         for (int i = 0; i < ifaces.length; i++) { 
              ...
              * 如果存在多张网卡包括虚拟机的网卡，默认只取第一张网卡的MAC地址，如果要返回所有的网卡（包括物理的和虚拟的）则可
              * 以修改方法的返回类型为数组或Collection  * ，通过在for循环里取到的多个MAC地址。  */  
    ...
    public void testNetIfList() throws Exception {  
        ...
        for (int i = 0; i < ifNames.length; i++) { 
        	   ...
        	   System.out.print("\nname = " + name);// 网络设备名  
        	   System.out.print("Address = " + ifconfig.getAddress());// IP地址  
        	   System.out. print("Netmask = " + ifconfig.getNetmask());// 子网掩码  
        	   if ((ifconfig.getFlags() & 1L) <= 0L) {  
        		   System.out.print("!IFF_UP...skipping getNetInterfaceStat");  
        		   /*;*/  
                   continue;  
               } 
        	   NetInterfaceStat ifstat = sigar.getNetInterfaceStat(name);  
        	   System.out.print("RxPackets = " + ifstat.getRxPackets());// 接收的总包裹数   
    		   System.out.print("TxPackets = " + ifstat.getTxPackets());// 发送的总包裹数  
    		   System.out.print("RxBytes = " + ifstat.getRxBytes());// 接收到的总字节数  
    		   System.out.print("TxBytes = " + ifstat.getTxBytes());// 发送的总字节数  
    		   System.out.print("RxErrors = " + ifstat.getRxErrors());// 接收到的错误包数  
    		   System.out.print("TxErrors = " + ifstat.getTxErrors());// 发送数据包时的错误数    
    		   System.out.print("RxDropped = " + ifstat.getRxDropped());// 接收时丢弃的包数  
    		   System.out.print("TxDropped = " + ifstat.getTxDropped());// 发送时丢弃的包数  
        }
    }
